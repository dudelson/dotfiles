...
#+BEGIN_SRC emacs-lisp
  (setq-default
     org-tag-faces
      '(("task" . (:foreground "#2aa198" :weight bold))
        ("pkb" . (:foreground "chartreuse" :weight bold))
        ("diary" . (:foreground "yellow" :weight bold))
        ("work" . (:foreground "cyan" :weight bold))
        ("unsorted" . (:foreground "#6c71c4" :weight bold))
        ("hl" . (:foreground "DeepPink1" :weight bold))
        ("^[A-Z].*[a-z].*" . (:foreground "orange" :weight bold))
        ))
#+END_SRC
* agenda
    ...
    (setq
      ...
      `(("t" "TODO" entry (file+headline ,(concat du/gtd-directory "/current.org") "tasks")
        ...
        ("p" "project" entry (file+headline ,(concat du/gtd-directory "/current.org") "projects")
          (file "/home/david/s/doc/kb/3 Resources/_templates/project.template.org") :empty-lines 1)
