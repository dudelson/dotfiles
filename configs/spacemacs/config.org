...
#+BEGIN_SRC emacs-lisp
  ...
  (defun du/toggle-planner ()
    ...
    (if du/toggle-planner-enabled
      ...
      (find-file "~/s/doc/kb/0 Inbox/INBOX.org"))
    ;; toggle the thingy
  ...
  (defun du/retangle-reload ()
    (interactive)
    (let ((src "/home/dudelson/.dotfiles/configs/spacemacs/config.org")
          (dst "/home/dudelson/.dotfiles/configs/spacemacs/config.el"))
      (if (file-exists-p src)
          (org-babel-tangle-file src dst)
        (message "Could not find config.org")))
    (let ((src "/home/dudelson/.dotfiles/configs/spacemacs/org-config.org")
          (dst "/home/dudelson/.dotfiles/configs/spacemacs/org-config.el"))
      (if (file-exists-p src)
          (org-babel-tangle-file src dst)
        (message "Could not find org-config.org")))
    (dotspacemacs/sync-configuration-layers)
    (let ((f "/home/dudelson/.dotfiles/configs/spacemacs/config.el"))
      (if (file-exists-p f)
          (load-file f)
        (message "Could not find config.el")))
    (let ((f "/home/dudelson/.dotfiles/configs/spacemacs/org-config.el"))
      (if (file-exists-p f)
          (load-file f)
...
I was too lazy to manually disable every code block here, so I just moved this
section to the end so that the errors don't prevent other code from loading.
** basic config
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; cheap hack to allow emacspeak to be turned on/off, since emacspeak does
  ;; not provide a means of disabling itself
  (defvar du/is-emacspeak-enabled t
    "Whether emacspeak is enabled or not")

  (defun du/emacspeak ()
    "Toggle emacspeak on/off"
    (interactive)
    (setq du/is-emacspeak-enabled (not du/is-emacspeak-enabled))
    (if (or
         (and du/is-emacspeak-enabled dtk-quiet)
         (and (not du/is-emacspeak-enabled) (not dtk-quiet)))
        (progn
          (dtk-toggle-quiet 1)
          ;; remove the buffer-local binding in the current buffer
          (kill-local-variable 'dtk-quiet)))
    (if (or
          (and du/is-emacspeak-enabled (not emacspeak-use-auditory-icons))
          (and (not du/is-emacspeak-enabled) emacspeak-use-auditory-icons))
        (progn
          (emacspeak-toggle-auditory-icons 1)
          ;; remove the buffer-local binding in the current buffer
          (kill-local-variable 'emacspeak-use-auditory-icons)))
    (message "TURNED EMACSPEAK %s" (if du/is-emacspeak-enabled "on" "off")))

  ;; turn emacspeak OFF initially
  (du/emacspeak)
  ...
  ;; spacemacs keybindings for emacspeak
  (spacemacs/declare-prefix "ae" "emacspeak")
  (spacemacs/set-leader-keys
    "ae SPC" 'du/emacspeak
    "aes" 'dtk-stop

    "aec" 'emacspeak-speak-char
    "aew" 'emacspeak-speak-word
    "ael" 'emacspeak-speak-line
    "aep" 'emacspeak-speak-paragraph
    "aer" 'emacspeak-speak-region
    "aeb" 'emacspeak-speak-buffer
    "aef" 'emacspeak-speak-buffer-filename
    "aeh" 'emacspeak-speak-help
    "aem" 'emacspeak-speak-modeline
    "aea" 'emacspeak-speak-message-again
    )

  (spacemacs/declare-prefix "aet" "toggles")
  (spacemacs/set-leader-keys
    "aeta" 'emacspeak-toggle-auditory-icons
    "aetm" 'emacspeak-toggle-speak-messages
    "aetp" 'dtk-toggle-punctuation-mode
    "aetC" 'dtk-toggle-allcaps-beep
    "aetk" 'emacspeak-toggle-character-echo
    "aetw" 'emacspeak-toggle-word-echo
    "aetl" 'emacspeak-toggle-line-echo)
#+END_SRC

#+RESULTS:

** fix hydra
From emacspeak 46, the hydra hints buffer was being read literally, which
preventing the audio interface to the hydra from being useful. I traced the
breaking change to [[https://github.com/tvraman/emacspeak/commit/320fc1ccfb3aea2de3390108b976b4adbdab610f?diff=unified][this commit]], which removes an advice on ~lv-message~. I guess
the emacspeak author doesn't use this for hydra hints, but this is how all
hydras work in spacemacs, so I have pasted the removed advice here and updated
it to work with the latest version of emacspeak.
#+BEGIN_SRC emacs-lisp
  (defadvice lv-message (after emacspeak pre act comp)
    "provide spoken feedback if idle, and hydra-is-helful is t."
...
#+END_SRC
** org-mode
Spacemacs transient state for navigating org files via the emacspeak audio
interface alone.
#+BEGIN_SRC emacs-lisp
  (defun du/emacspeak-speak-org-current-heading ()
    (interactive)
    (let* ((heading (org-element-at-point))
           ;; here we should check that the element we got back was a headline
           ;; but I'll skip for now
           (title (getf (nth 1 heading) :title)))
      (dtk-speak title)))

  (defun du/emacspeak-speak-org-next-heading ()
    (interactive)
    (org-forward-element)
    (du/emacspeak-speak-org-current-heading))

  (defun du/emacspeak-speak-org-prev-heading ()
    (interactive)
    (org-backward-element)
    (du/emacspeak-speak-org-current-heading))

  (defun du/emacspeak-speak-org-heading-tags ()
    (interactive)
    (let ((tags (org-get-tags-at)))
      (dtk-speak (mapconcat 'identity tags " "))))

  (defun du/org-parse-body-do (begin end action-alist)
    (let ((next begin)
          (e))
      (while
        (progn
          ...
          (setq
           action
            (alist-get
             (nth 0 e)
             action-alist
             (lambda (begin_ end_) nil)))
          (funcall action next (getf (nth 1 e) :end))
          (setq next (getf (nth 1 e) :end))
          ;; (pcase (nth 0 e)
          ;;   ('drawer (message "This is a drawer"))
          ;;   ('planning (message "This is a planning block"))
          ;;   ('paragraph (message "This is a paragraph"))
          ;;   (- (message "This is the catch-all")))
          (< next end)))))

  (defun du/emacspeak-speak-org-heading-body ()
    (interactive)
    (let* ((heading (org-element-at-point))
           (action-alist
            '((drawer .
               (lambda (begin end)
                 (message "This is a drawer")))
              (planning .
               (lambda (begin end)
                 (message "This is a planning block")))
              (paragraph .
               (lambda (begin end)
                 (message "This is a paragraph"))))))
      (du/org-parse-body-do
       (getf (nth 1 heading) :contents-begin)
       (getf (nth 1 heading) :contents-end)
       action-alist)))

  (defhydra du/emacspeak-org-nav
    (:body-pre (emacspeak-hydra-body-pre "This is a test")
               :pre emacspeak-hydra-pre
               :post emacspeak-hydra-post)
    "Audio interface for org-mode navigation"
    ("." du/emacspeak-speak-org-current-heading "Current heading")
    ("n" du/emacspeak-speak-org-next-heading "Next heading")
    ("p" du/emacspeak-speak-org-prev-heading "Prev. heading")
    ("u" ??? "Parent heading")
    ("d" ??? "Drill down")
    ;; ------------------------------------
    ("b" du/emacspeak-speak-org-heading-body "Speak body text")
    ("t" du/emacspeak-speak-org-heading-tags "Speak tags")
    ("P" ??? "Speak properties")
    ("T" ??? "Speak timestamps")
    ("L" ??? "Speak logbook")
    ("?" ??? "Speak help"))
  ...
  (spacemacs/set-leader-keys-for-major-mode
    'org-mode "SPC" 'du/emacspeak-org-nav/body)
  #+END_SRC
