...
local tooltip
local state = {}
local current_day_format = '<b><span color="#ff0000">%s</span></b>'
...
function displayMonth(month,year,weekStart)
  ...
  for d=1,mthDays do
    ...
    if d < 10 then x = " " .. x end
    lines = lines .. "  " .. x
    writeLine = writeLine + 1
  end
  if stDay + mthDays < 36 then lines = lines .. "\n" end
  if stDay + mthDays < 29 then lines = lines .. "\n" end
...
function cal.register(mywidget, custom_current_day_format)
  ...
  if not tooltip then
    ...
    function tooltip:update()
      ...
      state = {month, year}
      tooltip:set_markup(string.format('<span foreground="white" font_desc="monospace">%s</span>', displayMonth(month, year, 1)))
    end
    tooltip:update()
  end
...
function switchMonth(delta)
  ...
  local text = string.format('<span foreground="white" font_desc="monospace">%s</span>', displayMonth(state[1], state[2], 1))
